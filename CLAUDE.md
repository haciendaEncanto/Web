...
3. **Playlist cliente** (`/portal/playlist`) — tabla `playlists`, cliente agrega canciones con artista y género.

4. **Distribución de invitados por mesa** (`/portal/invitados`) — tabla `guest_tables`, cliente organiza invitados en mesas con nombre y capacidad.

5. **Conectar dominio hacienda-encanto.com a Vercel** — cuando el sitio esté listo para producción. Cuenta Vercel ya aprobada.

6. **Videos empresarial y revelación** — archivos los provee el cliente. Subirlos desde `/editor/videos` (upload ya funcional), activar para la página correspondiente.

7. **Formulario de contacto redirige a WhatsApp** — cambiar el flujo del formulario de contacto público para que, en vez de (o además de) guardar el mensaje en `contact_messages`, redirija/abra WhatsApp con el mensaje prellenado.
